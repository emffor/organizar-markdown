...
export function CombinedOutputPanel({
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 overflow-y-auto p-5 sm:p-7 ${theme === 'dark' ? 'bg-[#0b1118]' : 'bg-slate-50/80'}`}>
        {isLoading ? (
          <p className={`m-0 text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Carregando conteudo...</p>
        ) : items.length > 0 ? (
          <article
            ...
            {items.map((item, index) => (
              ...
              >
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <p className="m-0 text-xs font-medium uppercase tracking-[0.22em] text-slate-400">
          ...
          <div
            className={`flex h-full min-h-[320px] w-full items-center justify-center rounded-[1.35rem] border border-dashed px-6 text-center ${
              theme === 'dark'
                ? 'border-slate-700/80 bg-[#0c1219]'
                : 'border-slate-300 bg-white'
            }`}
          >
            <p className={`m-0 text-sm leading-6 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
              O preview final vai aparecer aqui, seguindo exatamente a ordem dos cards e carregando as imagens do markdown.
            </p>
          </div>
        )}
      </div>
